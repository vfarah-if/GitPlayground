...
1. This assumes you have Visual studio or VSCode. The solution used .Net framework 4.6.2 but I could have easily done this in .net core, just chose to do it this way as I was testing FLURL and Bddfy for a project that was not using .net core and so I continued down this route.
	...
	10. My react project is going to be developed as a typescript project. For setup and project details see more within [src/react-accident-statistics](src/react-accident-statistics). I have started developing the first component and it went well, with one small snag and that was related to the relative pathing of components and make sure you use relative rather than full path. The architecture promotes lots of small components, the same with Angular, however Angular has a more sophisticated binding mechanism, with *directives* and *pipes* which are like components in themselves and Angular supports lots of built in  structures to help with date formatting, JSON and the list goes on, lending itself more to not needing smaller components as it will still be very readable and maintainable. I need to warn you that the react typescript CLI has small issues and creates far less than the Angular CLI, probably on purpose, like no integration or e2e setup and errors with basic things you will need to search solutions for to get going. The Angular CLI have raised the bar for the React CLI, but to be honest both have spoilt developers wrotten by giving us frameworks that allow us to focus mostly on our business needs. Not too long ago, I was creating Grunt and Gulp scripts to automate a lot of what I get for nothing, so Kudos to the guys who open sourced these great CLI's as they promote consistency and help us develop quicker.
	...
	12. Docker and containers are a very useful way to create production builds to containers, testing and sealing the intended environment for everyone to use as expected with no unknown environmental issues. I chose to venture down setting up Docker as I mostly used Vagrant and Virtual box, and so I wanted to figure out how easy it is to utilise docker and finally deploy a test to AWS, Azure or Heroku, to host Node apps on their ideal operating system, which turns out to be Linux over windows or Mac. I also wanted to minimise the amount of effort people needed to setup MongoDb, that could be created and destroyed allowing people to automate this setup in one quick go without polluting their local environments. *docker-compose.yml* and *Dockerfile* can be found in the root of every project that supports docker, as well as more detailed descriptions of what was entailed in the docker implementation.
		...
		2. [docker-compose.yml](docker-compose.yml) contains a way of buildind all the projects and then running it locally without compiling or installing node or mongo by running `docker-compose up` and remember to change the env file within the [src/node-accident-statistics/.env](src/node-accident-statistics/.env) to allow mongodb to be setup with docker.
		3. [docker-compose-from-dockerhub.yml](docker-compose-from-dockerhub.yml) contains a faster option for downloading already built images I created `docker-compose -f docker-compose-from-dockerhub.yml up`. I utilised [docker multi stage builds](https://docs.docker.com/develop/develop-images/multistage-build/) to reduce the size of the final docker images and hosted *Angular* and *React* in an *Nginx* hosted docker container. The mongo also included [Mongo express](https://www.npmjs.com/package/mongo-express), if you dont't have Compass locally, so that can be viewed and the data will not be lost on the images when it is downloaded and setup, which makes it very suitable for hosting a database in a production environment, not that I think that is a good idea. You can vie mongo express on *localhost:8081*, with the ability to see what is in your *MongoDb*.
	...
	Docker does have some virtualization issues, known problems with HyperV and Virtualbox, and like I said works best on a Linux environment. The hardest part was understanding what it does on each image. `docker exec -it ae /bin/sh` or `docker exec -it ae /bin/bash` allowed me to get onto the image and find what had been created. `docker ps` alllowed me to query some summary information about what container was up and what ports it was exposed on. 
