...
describe('AccidentTitle', () => {
    ...
    it('should describe the two fatal acidents', () => {
        wrapper = enzyme.shallow(<AccidentTitle pagedAccidentStatistic={pagedData} severityOption={'Fatal'}/>)
        expect(pagedData).toBeTruthy();
    ...
    it('should mention the from and to date of the fatal accident', () => {
        wrapper = enzyme.shallow(<AccidentTitle pagedAccidentStatistic={pagedData} severityOption={'Fatal'} from={fromDate} to={toDate}/>)
    ...
    it('should mention the custom order by location descending', () => {
        wrapper = enzyme.shallow(<AccidentTitle pagedAccidentStatistic={pagedData} severityOption={'Fatal'} from={fromDate} to={toDate} orderByOption={'LocationDescending'}/>)
        expect(wrapper.find('h1').text()).toContain('ordered by locationdescending');
    });

    it('should mention the custom order by location descending', () => {
        wrapper = enzyme.shallow(<AccidentTitle pagedAccidentStatistic={pagedData} severityOption={'Serious'} from={fromDate} to={toDate} orderByOption={'BoroughAscending'}/>)
