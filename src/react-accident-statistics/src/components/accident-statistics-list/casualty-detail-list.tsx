...
export default class CasualtyDetailList extends React.PureComponent<CasualtyDetailListProps, any> {
    public render() {
        ...
        return (
            <dl>
                {casualties.map((casualty: Casualty, index: number) =>
                    <dt>
                        <span><FontAwesomeIcon icon="ambulance" transform="shrink-3" color="orange"/> Casualty {index + 1} of {casualty.severity ? this.severityDescription(casualty.severity) : 'unknown'} severity, aged {casualty.age}
                        </span>
                    </dt>
                )}
            </dl>
